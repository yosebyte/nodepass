...
NodePass创建一个使用TLS加密保护的安全隧道用于控制信号，并使用高效的TCP/UDP连接进行数据传输。它有两种互补的运行模式：
...
在服务器模式下，NodePass:
1. 在`tunnel_addr`上监听TLS隧道连接
2. 在`target_addr`上监听传入的TCP和UDP流量
3. 当`target_addr`收到连接(通过TCP)或数据(通过UDP)时，通过安全隧道向客户端发送信号
4. 为每个连接创建数据通道并将流量转发给客户端
...
在客户端模式下，NodePass:
1. 使用TLS连接到服务器的隧道端点(加密控制通道)
2. 通过此安全通道监听来自服务器的信号
3. 当收到信号时，连接到服务器的远程端点
...
1. **服务器模式**:
   - 设置监听器: 隧道(TLS加密)、远程(未加密)和目标(适用于TCP和UDP)
   - 接受隧道端点上的传入连接
   - 当客户端连接到目标(通过TCP)或发送数据(通过UDP)时，通过安全隧道向客户端发送信号
   - 然后客户端建立到远程端点的连接(未加密)
   - 在目标和远程连接之间交换数据
...
2. **客户端模式**:
   ...
   - 建立到本地目标地址的连接(根据需要使用TCP或UDP)
   - 在远程和本地目标连接之间交换数据

3. **协议支持**:
   - **TCP**: 具有持久连接的完整双向流
   - **UDP**: 具有可配置缓冲区大小和超时的数据报转发
   - 两种协议使用相同的信号机制，但有不同的处理模式
...
### 1. 关注点分离
代码库在以下方面保持清晰分离:
- **命令层**: 处理用户输入和配置(在`cmd/nodepass`中)
- **服务层**: 实现核心客户端和服务器逻辑(在`internal`中)
