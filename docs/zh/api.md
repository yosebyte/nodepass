...
2. **持久化存储**：API Key与其他实例配置一起保存在`nodepass.json`文件中
...
NodePass主控模式现在支持使用JSON格式进行实例持久化。实例及其状态会保存到与可执行文件相同目录下的`nodepass.json`文件中，并在主控重启时自动恢复。
...
虽然不推荐手动修改`nodepass.json`文件，但在特殊情况下可能需要这样做。请遵循以下最佳实践：

**编辑前的准备工作：**
1. **停止主控服务**：在编辑前必须先停止NodePass主控进程
2. **备份原文件**：编辑前制作`nodepass.json`的备份副本
3. **验证JSON格式**：使用JSON验证工具确保格式正确
4. **了解字段含义**：确保理解每个字段的作用和有效值
...
```json
{
  "instance_id": {
    "id": "实例唯一标识符",
    "type": "实例类型 (client/server 或特殊值 ********)",
    "status": "实例状态 (running/stopped/error)",
    "url": "实例URL或API Key",
    "tcprx": 0,    // TCP接收字节数
    "tcptx": 0,    // TCP发送字节数  
    "udprx": 0,    // UDP接收字节数
...
**编辑注意事项：**
- `id`字段必须与键名匹配
- `type`字段只能是"client"、"server"或特殊API Key标识"********"
- `status`字段建议设为"stopped"，避免启动时状态不一致
- URL格式必须符合scheme://host:port/target:port的规范
- 流量统计字段必须为非负整数
- API Key实例（id为"********"）的`url`字段包含实际的API密钥
- 保持JSON格式的完整性，注意逗号和引号的正确使用
...
由于NodePass现在使用JSON格式持久化存储实例状态，实例ID在主控重启后**不再发生变化**。这意味着：
